...
const verDevolucion = ref(false);
const ventaADevolver = ref(null);

const abrirDevolucion = (v) => {
    cerrarMenu();
    ventaADevolver.value = v;
    verDevolucion.value = true;
};
...
const confirmarAnulacion = async (v) => {
    ...
    const { value: motivo, isConfirmed } = await Swal.fire({
        ...
        icon: 'warning',
        input: 'text',
        inputPlaceholder: 'Motivo (ej: Error en la carga, Cliente desistió...)',
    ...
    if (!isConfirmed || !motivo?.trim()) return;
    realizarAnulacion(v.id, motivo.trim());
};
</script>
            ...
            <div v-if="loadingVentas" class="bg-white shadow-xl rounded-2xl border border-slate-100 p-4 mb-4">
                <div class="p-4 space-y-4">
                    <div v-for="n in 5" :key="n" class="flex items-center gap-4 animate-pulse">
                        ...
                        <div class="h-4 w-4 bg-slate-100 rounded-full ml-auto"></div>
                    </div>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-if="ventas.data.length === 0">
                                <td colspan="6" class="p-10">
                                    <AlertaAyuda>No se encontraron ventas con esos filtros.</AlertaAyuda>
                                </td>
                            </tr>
                            ...
                            >
                                ...
                                <td class="p-4 text-center">
                                    ...
                                    <span v-if="v.estado === 'Completada' && v.detalles?.some(d => Number(d.cantidad_devuelta) > 0)"
                                        class="ml-1 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest bg-amber-100 text-amber-700 border border-amber-200"
                                    >
                                        Dev.
                                    </span>
                                </td>
                                ...
                                <td class="p-4 text-center">
                                    <DropdownAcciones :abierto="menuAbierto === v.id" @close="menuAbierto = null">
                                        <template #trigger>
                                        ...
                                        <a :href="route('ventas.imprimir', v.id)" target="_blank" @click="cerrarMenu()" class="w-full text-left px-4 py-2.5 text-xs font-bold text-emerald-600 hover:bg-emerald-50 flex items-center gap-3 transition-colors">
