...
class VerificarEstadoCuenta
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->routeIs('login', 'register', 'logout', 'cuenta.suspendida', 'pending.approval', 'elegir.sucursal', 'elegir.sucursal.store', 'admin.comercios.*', 'impersonate.*', 'mercadopago.notificacion', 'viumi.webhook', 'tienda.*', 'api.tienda.*', 'cliente.*')) {
            return $next($request);
        }
